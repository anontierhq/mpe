...
MPE is a Rust service that pulls **jobs** from **RabbitMQ**, runs **video** and **image** tasks in parallel, and writes outputs to disk (**HLS / MPEG-DASH** for video; validated, privacy-scrubbed stills for images). **Redis** holds live **job** and **per-task** status for observers and dashboards.
...
- **Image pipeline** — ffprobe validation, **exiftool** removal of GPS/location metadata, copy to the task output directory (original filename).
...
1. **Install** [Rust](https://rustup.rs), **ffmpeg** / **ffprobe**, **Shaka Packager** as `packager`, and **exiftool** if you use image tasks (see [Getting started](docs/getting-started.md#tooling)).
...
| [Image metadata](docs/image-metadata.md) | exiftool tags cleared for image tasks |
